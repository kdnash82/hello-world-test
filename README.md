...
```bash
./mvnw package  
java -jar target/hello-world-test-0.0.1-SNAPSHOT.jar --app.message=Test
```
...
```bash
tanzu apps workload create hello-world-test -f config/workload.yaml \
  --local-path . \
  --source-image harbor.az.dlpvmw.net/tap/hello-world-test-source \
  --type web
```
...
```
tanzu apps workload get hello-world-test
```
